...
/** The flash device to program. */
struct device *boot_flash;
 ...
 */
static int
boot_image_check(struct boot_image_location *loc)
{
	return bootutil_img_validate(loc->bil_address + BOOT_FLASH_BASE);
}
 ...
 */
int
boot_go(struct device *flash_dev,
	const struct boot_req *req, struct boot_rsp *rsp)
{
	int slot;
	int rc;

	boot_flash = flash_dev;
...
#if 0
	/* TODO: This code probably won't ever run, since the
	 * boot_select_image_slot() code assumes the magic header has
	 * been written.  However, this logic isn't quite right,
	 * either. */
	if (slot) {
		boot_state.idx = 0;
		boot_state.state = 0;
